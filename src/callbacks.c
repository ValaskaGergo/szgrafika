...
void search_closest_raptor(World *world, Vertex camera_position)
{
	int raptor_id;
	float min_distance = MAX_MOVING_DISTANCE;
	int i;

	for (i = 0; i < RAPTOR_LENGTH; i++)
	{
		float distance = sqrt(pow((camera_position.x - world->raptors[i].position.x), 2) + pow((camera_position.z - world->raptors[i].position.z), 2));
...
void keyboardUp(unsigned char key, int x, int y)
{
	...
	case 'x':
		if (move_raptor_id == -1)
		{
			search_closest_raptor(&world, camera.position);
		}
		else
		{
			move_raptor_id = -1;
	...
	case 'q':
		if (move_raptor_id != -1)
		{
			world.raptors[move_raptor_id].rotation += 90.0f;
		}
		break;
	case 'e':
		if (move_raptor_id != -1)
